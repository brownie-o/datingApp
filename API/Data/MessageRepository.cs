...
public class MessageRepository(AppDbContext context) : IMessageRepository
{
  ...
  public async Task<PaginatedResult<MessageDto>> GetMessagesForMember(MessageParams messageParams)
  {
    ...
    // project to MessageDto
    var messageQuery = query.Select(MeassageExtensions.ToDtoProjection());
  ...
  {
    // mark the messages as read if the current user is the recipient and the message has not been read yet
    ...
    // get the messages between the current user and the recipient, ordered by message sent date
    return await context.Messages
      ...
      .Select(MeassageExtensions.ToDtoProjection())
      .ToListAsync();
  }
