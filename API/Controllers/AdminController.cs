...
public class AdminController(UserManager<AppUser> userManager, IUnitOfWork uow, IPhotoService photoService) : BaseApiController
{
  ...
  public async Task<ActionResult<IList<string>>> EditRoles(string userId, [FromQuery] string roles)
  {
    ...
    // make the selected roles into an array
    ...
    return Ok(await userManager.GetRolesAsync(user)); //returning IList<string> of the user's roles
  }
  ...
  public async Task<ActionResult> ApprovePhoto(int photoId)
  {
    ...
    // Load the member with related User entity so we can safely update both image urls.
    ...
    if (!await uow.Complete()) return BadRequest("Failed to approve photo");
